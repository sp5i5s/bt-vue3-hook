...
export const useCache = async (options: iOptions) => {
  ...
  if (!session.getItem(options.key)) {
    if (options.promiseFn) {
      let promiseResult = await utils.promiseHandle(options.promiseFn, options.params || {})
      result = JSON.stringify({ source: promiseResult, created: new Date().getTime() });
  ...
  if (options?.updateTime) {
    ...
    if (sourceFormatObject.created + options.updateTime < new Date().getTime()) {
